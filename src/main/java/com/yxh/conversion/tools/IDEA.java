...
public class IDEA {
    ...
    public String getEncString(String strMing) {
        try {
            byte[] bytesss = IdeaEncrypt(bytekey, strMing.getBytes(), true);
            return byte2hex(bytesss);
        } catch (Exception e) {
            e.printStackTrace();
            return "";
        }
    ...
    public String getDesString(String strMi) {
        try {
            byte[] bytesss = IdeaEncrypt(bytekey, hex2byte(strMi.getBytes()), false);
            StringBuffer str = new StringBuffer();
            for (int i = 0; i < bytesss.length; i++) {
                if (bytesss[i] < 9 && bytesss[i] > 0) {
                    break;
                }
                str.append(new Character((char) bytesss[i]));
            }
    ...
    public static void main(String[] args) {
        ...
        idea.getKey("xianhe_yan");// 生成密匙

        String strEnc = idea.getEncString("yanxianhe");// 加密字符串,返回String的密文
